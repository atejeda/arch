...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch dmenu
    , ((modm,               xK_p     ), spawn "dmenu_run -fn 'Hack-10'")
    ...
    -- Screenshots
    , ((modm .|. shiftMask, xK_s     ), spawn "maim -s --format png /dev/stdout | xclip -selection clipboard -t image/png -i")
...
--
myLayout = avoidStruts (tiled ||| Mirror tiled ||| Full ||| multiCol [1] 1 0.01 (-0.5))
  where
...
main = do
    xmproc <- spawnPipe "xrdb -merge ~/.Xresources; killall xmobar || xmobar -x 0"
    xmonad $ docks defaults {
        logHook = dynamicLogWithPP $ xmobarPP {
            ppOutput = hPutStrLn xmproc
          , ppTitle = xmobarColor xmobarTitleColor "" . shorten 5 
